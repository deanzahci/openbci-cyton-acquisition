...
def start_csv_recording(filename=None):
    """
    Start recording data to CSV file.
    If filename is None, generates timestamp-based filename.
    """
    global CSV_RECORDING, CSV_FILE, CSV_WRITER
    
    if CSV_RECORDING:
        print("CSV recording is already active.")
        return False
    ...
    try:
        with CSV_LOCK:
            ...
            # Write header
            header = ['timestamp'] + [f'channel_{i+1}' for i in range(CHANNEL_COUNT)]
...
def write_sample_to_csv(timestamp, data):
    """Write a single sample to CSV file."""
    global CSV_RECORDING, CSV_WRITER, CSV_FILE
    
    if CSV_RECORDING and CSV_WRITER:
        try:
            with CSV_LOCK:
                row = [timestamp] + data.tolist()
...
def stream_callback(sample):
    ...
    write_sample_to_csv(timestamp, scaled_data)
...
def generate_random_data():
    ...
    while True:
        ...
        # Write to CSV if recording
        write_sample_to_csv(current_time, random_sample)
